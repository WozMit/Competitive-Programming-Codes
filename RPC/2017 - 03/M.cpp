...
void Sieve(){
	...
	for(int i=3; i*i<MAX; i+=2)
		if(prime[i]) for(ll j = (ll)i*i; j<MAX; j+=2ll*i) prime[j] = false;
}
...
bool IsPrime(ll n){ return n < (ll)MAX ? prime[n]:Miller_Rabin(n); }
...
int main(){
	...
	while(te--){
		...
		while(n > 1ll){
			...
			while(!IsPrime(pf)) pf = Pollards_Rho(n);
			while(n%pf == 0) n /= pf, V.push_back(pf);
		}
		sort(V.begin(), V.end());
